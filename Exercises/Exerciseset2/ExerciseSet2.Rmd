...
* 1. We need at least 6 factors. With 6 factors we explain 52.29% of the variance as we can see from following table <br>
`r Variance_Explained_Table[,"Cumulative pct of explained variance"]`
* 2. Highest values are 0.93 as we can see in following vector, showing the values of comp1: <br>
`r Rotated_Factors[,"comp 1"]`
<br>
*3. Comp1: North American Bonds; <br>
    Comp2: Currencies; <br>
    Comp3: European Stock Indexes; <br>
    Comp4: North American Stock Indexes; <br>
    Comp5: European Bonds; <br<
    Comp6: Oil; <br>
    Comp7: Metals; <br>
    Comp8: Agricultural Commodities; <br>
    Comp9: Precious Metals; <br>
    Comp10: Asian Stock Indexes; <br>
*4. 
Crisis scree plot and principal components before crisis:
```{r echo=TRUE, tidy=TRUE}
...
```
Crisis scree plot and principal components during crisis:
```{r echo=TRUE, tidy=TRUE}
...
```
*<br>
* 5. Comparing Components before and during crisis, we found several differences. <br>
Before Crisis: <br>
```{r echo=TRUE, tidy=TRUE}
...
We can see that components weigths are quite stable even during the crisis, even though some weights change a little bit. Generally speaking group of commodities keep to be correlated in the same way before and during the crisis. <br>
...
* 1. The code above calculates the moving averages using function "ma" with different windows defined by the vector "MAfreq". This vector is used both to define the lenght to be used and how many different values will be used.
Averages are scaled to obtain weights with total sum of 1 each day. Then weights are multiplied by futures prices to define how much to invest on each future each day. Finally, cumulative result is calculated as sum cross-future for each day. <br>
