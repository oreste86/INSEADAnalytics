...
**Your Answers here:**
<br>
Answer1: dataSet1.R has a getdata.fromscratch to download data from Yahoo website. "mytickers" variable include all the stocks we want to download. If we want to download an additional stock, we just have to put its name in this variable.
<br>
Answer2:Cumulative return: round(100*sum(StockReturns[,2]),1)%;
Average daily returns: round(100*mean(StockReturns[,2]),3)%;
Standard deviation of the daily returns: round(100*sd(StockReturns[,2]),3)%;
<br>
Answer3: Yes,we can add YHOO to "mytickers" and run dataSet1.R again
<br>
...
<br>
Answer1: <br>
nrow(transposedData) --> number of rows; <br>
ncol(transposedData) --> number of columns.
<br>
Answer2:
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
transposed_port = apply(transposedData,2,mean)
names(transposed_port) <- colnames(transposedData)
pnl_plot(transposed_port)
```
<br>
<hr>
<hr>
...
**Your Answers here:**
<br>
Answer1: modify startDate in dataSet1.R to 2001-01-01 and run again with command source("dataSet1.R")
<br>
Answer2:modify mytickers in dataSet1.R to add new stocks (e.g. YHOO), run again with command source("dataSet1.R"). Then we can repeat function apply(StockReturns,1,mean) to compute portfolio return since it's equal weighted.
<br>
<br>
...
**Your Answers here:**
<br>
Answer1: Result is 20 because for each row myData is not equal to StockReturns. myData is a vector where each element is a concatenation of date, yahoo and apple stocks while StockReturns has each value in a different column. To create similar structure we should use "," as separator when importing twentydays.csv
<br>
<br>
<br>
